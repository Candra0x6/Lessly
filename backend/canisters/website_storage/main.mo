...
/// WebsiteStorage canister handles storage and retrieval of website assets
actor WebsiteStorage {
  ...
  private var chunks = HashMap.HashMap<(AssetId, Nat), Blob>(
    0,
    func(a, b) {
  ...
  system func postupgrade() {
    ...
    chunks := HashMap.fromIter<(AssetId, Nat), Blob>(
      chunkEntries.vals(),
      0,
      func(a, b) {
        Text.equal(a.0, b.0) and a.1 == b.1
  ...
  /// Returns true if the user has access to the specified project
  private func hasAccess(userId : UserId, projectId : ProjectId) : Bool {
    switch (projectAccess.get(projectId)) {
      case (?users) {
        switch (Array.find<UserId>(users, func(id) { id == userId })) {
          case (?_) { true };
          case null { false };
        };
      };
  ...
  /// Returns true if access was successfully configured
  ...
  /// Get asset metadata - query function that doesn't modify state
  /// Returns the asset metadata or an error
  ...
  /// Get a chunk of asset data - query function that doesn't modify state
  /// Returns the chunk data or an error
  ...
  /// Get all assets for a project - query function that doesn't modify state
  /// Returns an array of assets for the specified project
  ...
  /// Get assets for a specific project version - query function that doesn't modify state
  /// Returns an array of assets for the specified project version
