...
## Purpose
The Website Renderer Canister is responsible for serving website content to visitors. It handles HTTP requests, retrieves the appropriate assets from the storage canister, and dynamically renders websites based on URL routes.
...
|--------|------|-------------|
| `http_request` | Query | Handles HTTP requests and serves website content |
| `http_streaming_callback` | Query | Provides streaming for large assets |
| `linkDomain` | Update | Links a custom domain to a project |

## Integration Points
- Interacts with Website Storage Canister to retrieve assets
- Interacts with Project Management Canister to check project existence and status

## Performance Considerations
- Uses streaming for large assets
- Implements caching headers for better performance
- Optimizes URL routing for efficient asset retrieval 