...
export default function TemplateGrid({ templates, selectedTemplate, onSelect }: TemplateGridProps) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <motion.div
        ...
        className={`relative rounded-lg border-2 overflow-hidden cursor-pointer transition-all ${
          selectedTemplate === "blank" ? "border-blue-500 shadow-md" : "border-gray-200 dark:border-gray-800"
        }`}
      ...
      >
        ...
        <div className="p-4">
          <h3 className="font-medium text-gray-900 dark:text-white">Blank Canvas</h3>
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Start from scratch with a clean slate</p>
        </div>
      ...
      {templates.map((template) => (
        <motion.div
          ...
          whileHover={{ y: -5 }}
          className={`relative rounded-lg border-2 overflow-hidden cursor-pointer transition-all ${
            selectedTemplate === template.id ? "border-blue-500 shadow-md" : "border-gray-200 dark:border-gray-800"
          }`}
